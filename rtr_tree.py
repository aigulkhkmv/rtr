...
class LemonTree():
    ...
    def go_to_random_child(root_number):
        list_of_children = G.successors(LemonTree.go_to_child)
        return list_of_children[random.randint(0, len(list_of_children) - 1)]
    ...
    if G.nodes[node_number]["terminal"] != True:
        ...
        if len(new_mols_from_pred[0]) == 0:
            ...
            for j2 in range(len(new_mols_from_pred[0])):
                ...
                # and 2) if list of hashes != list of new list of hashes; if we have one prediction and this prediction in node we contionue
                if rollout == False:
                    if LemonTree.go_to_child(node_number) == []:
                        ...
                        "list_of_molecules"] == copy_of_list_of_molecules:
                        node_attrs_7 = {new_node_number: {"expanded": True}}
...
def search(node_number, random = False): #traverse the tree
    ...
    if G.nodes[new_node_number]["expanded"] == True and G.nodes[new_node_number]["terminal"] == False:
        search(new_node_number, random)
    elif G.nodes[new_node_number]["expanded"] == False:
